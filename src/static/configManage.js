...
export const configManage = {
  ...
  getConfigSation: function () {
    http.sysInfolist().then(res => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].sysKey === 'permissionArr') {
          configManage.formInline.permissionArr = JSON.parse(data[i].sysValue)
        }
        if (data[i].sysKey === 'permissionDataArr') {
          configManage.formInline.permissionDataArr = JSON.parse(data[i].sysValue)
        }
        if (data[i].sysKey === 'dataPermissionWKTVal') {
          configManage.formInline.dataPermissionWKTVal = data[i].sysValue
        }
        if (data[i].sysKey === 'dataPermissionCodeVal') {
          configManage.formInline.dataPermissionCodeVal = data[i].sysValue
        }
      }
    })
  ...
  saveUpdataConfig: function (params) {
    http.sysSaveSysInfo(params).then(res => {
      if (res.data === 'true') {
        // @ts-ignore
        this.$message({
          type: 'success',
